	...
	function () {
		...
		$("#tabs").tabs({
			activate: function (event, ui) {
				...
				if (ui.newPanel.attr('id') == "Infos") {
					InitGauge();
				}
				if (ui.newPanel.attr('id') == "Pictures") {
					loadImage("/images/mire.jpg", 160, 120, '#img_snapshot');
				}
				if (ui.newPanel.attr('id') == "Rfid") {
					RefreshTagList();
				}
				if (ui.newPanel.attr('id') == "TTS") {
					RefreshVoiceList("#cbx_voice", 1);
				}
				if (ui.newPanel.attr('id') == "Sounds") {
					RefreshSoundList('#cbx_sound');
					RefreshRadioList();
				}
				if (ui.newPanel.attr('id') == "Update") {
					CheckUpdate();
					CheckPatch();
					CheckFirmware();
				}
