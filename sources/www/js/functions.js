	...
	$.getJSON("/cgi-bin/moods_list", function (data) {
		if (parseInt(data.return) == 0) {
			if (data.moods.length == 0) {
				$(id).empty();
				$(id).append('<option value="-1">No Moods</option>');
				$(id).prop("disabled", true);
...
function RefreshStoriesList() {
	...
	$('#cbx_stories').append('<option value="-1">Loading Stories ...</option>');
	$.blockUI({message: '<H2><img src="/images/ajax-loader.gif" />&nbsp;Reading story list'});
	$.getJSON("/cgi-bin/stories_list", function (data) {
		if (parseInt(data.return) == 0) {
			if (data.stories.length == 0) {
				$('#cbx_stories').empty();
				$('#cbx_stories').append('<option value="-1">No Stories</option>');
				$('#cbx_stories').prop("disabled", true);
			} else {
				$('#cbx_stories').empty();
				...
				for (var i = 0; i < data.stories.length; i++) {
					$('#cbx_stories').append('<option value="' + data.stories[i].id + '">' + CapitaliseFirstLetter(data.stories[i].text) + '</option>');
				}
			}
		}
	});
